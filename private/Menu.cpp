...
void Menu::PrintPreferences(const ActionPreferences& Preferences)
{
	...
	std::cout << "Chance to subscribe to random neighbor of neighbor's neighbor: " << Preferences.GetPreference(NodeActions::SubscribeNeighbor) << std::endl;
	std::cout << "Chance to unsubscribe from random neighbor: " << Preferences.GetPreference(NodeActions::UnsubscribeNeighbor) << std::endl;
